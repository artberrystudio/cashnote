...
export function SettingsScreen() {
  const {
    records, loadRecords, syncStatus, userId,
    userEmail, isAnonymous, signOut,
        ...
        { text: '취소', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              for (const k of cacheKeys) await AsyncStorage.removeItem(k);
              await loadRecords();
            } finally { setLoading(null); }
            appAlert('완료', '캐시가 정리되었습니다.');
          },
        },
      ]
    );
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (Platform.OS === 'web') {
        ...
        if (canShare) {
          await Sharing.shareAsync(path, { mimeType: 'application/json', dialogTitle: 'CashNote 백업 저장' });
        } else {
          appAlert('백업 완료', `파일 저장됨: ${path}`);
        }
      }
    } catch (e: any) {
      appAlert('오류', `백업 실패: ${e.message}`);
    } finally { setLoading(null); }
  };
  ...
  const handleImport = async () => {
    ...
    try {
        ...
        [
          { text: '취소', style: 'cancel' },
          {
            ...
            onPress: async () => {
              ...
              setLoading(null);
              appAlert('완료', `${newRecords.length}건 추가되었습니다.`);
            },
          },
        ]
      );
      ...
      setLoading(null);
      appAlert('오류', e.message ?? '파일을 읽을 수 없습니다.');
    }
  };
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      >
        ...
        <View style={[styles.syncBadge, { backgroundColor: sync.bg }]}>
          ...
          {userId ? <Text style={styles.syncUid} numberOfLines={1}>ID: {userId.slice(0, 8)}…</Text> : null}
        </View>
        ...
        {isAnonymous ? (
          ...
          <View style={styles.group}>
            ...
            <SettingsRow
              ...
              title="로그아웃" subtitle="다른 계정으로 전환하거나 로그아웃합니다"
              onPress={handleSignOut} loading={loading === 'signout'} danger
            />
          </View>
        )}
        ...
        <View style={styles.group}>
          <SettingsRow
            icon="refresh-outline" iconColor="#2563EB" iconBg="#DBEAFE"
            title="새로고침" subtitle="저장된 데이터를 다시 불러옵니다"
            onPress={handleRefresh} loading={loading === 'refresh'}
          />
          <Separator />
          <SettingsRow
            icon="trash-outline" iconColor="#D97706" iconBg="#FEF3C7"
            title="캐시 지우기" subtitle="임시 데이터를 정리합니다 (기록 유지)"
            onPress={handleClearCache} loading={loading === 'cache'}
          />
        </View>
        ...
        <View style={styles.group}>
          <SettingsRow
            icon="download-outline" iconColor="#059669" iconBg="#D1FAE5"
            title="백업 내보내기" subtitle="모든 기록을 JSON 파일로 저장합니다"
            onPress={handleExport} loading={loading === 'export'}
            badge={records.length > 0 ? `${records.length}건` : undefined}
          />
          <Separator />
          <SettingsRow
            icon="cloud-upload-outline" iconColor="#7C3AED" iconBg="#EDE9FE"
            title="백업 불러오기" subtitle="JSON 백업 파일에서 복원합니다"
            onPress={handleImport} loading={loading === 'import'}
          />
        </View>
        ...
        <View style={styles.group}>
          <SettingsRow
            icon="warning-outline" iconColor="#EF4444" iconBg="#FEE2E2"
            title="전체 데이터 삭제" subtitle="모든 수입 기록을 완전히 삭제합니다"
            onPress={handleDeleteAll} loading={loading === 'delete'} danger
          />
        </View>
...
interface RowProps {
  ...
  onPress: () => void; loading?: boolean; badge?: string; danger?: boolean;
}
function SettingsRow({ icon, iconColor, iconBg, title, subtitle, onPress, loading, badge, danger }: RowProps) {
  return (
...
const styles = StyleSheet.create({
  ...
  accountCard: {
    ...
    marginBottom: 24, alignItems: 'center', gap: 8,
  ...
  accountDesc: { fontSize: 13, color: '#9CA3AF', textAlign: 'center', lineHeight: 19 },
  accountBtns: { flexDirection: 'row', gap: 10, marginTop: 8, width: '100%' },
  accountBtnPrimary: {
    flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
    gap: 6, backgroundColor: '#059669', borderRadius: 12, paddingVertical: 12,
  },
  accountBtnPrimaryText: { fontSize: 14, fontWeight: '700', color: '#FFFFFF' },
  accountBtnSecondary: {
    flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
    gap: 6, backgroundColor: '#F0FDF4', borderRadius: 12, paddingVertical: 12,
